...
async def test_form_areas_to_zones(hass: HomeAssistant):
    ...
    areas_input = {CONF_HOME_AREA: "01", CONF_AWAY_AREA: "02"}
...
async def test_form_areas_add_another(hass: HomeAssistant):
    ...
    areas_input = {CONF_HOME_AREA: "01", CONF_AWAY_AREA: "02", CONF_ADD_ANOTHER: True}
...
async def test_form_zones_creates_entry(hass: HomeAssistant):
    ...
    flow = DMPCustomConfigFlow()
    flow.hass = hass
    flow.data = {"panel_name": "Test Panel", "ip": "192.168.1.100", CONF_ZONES: []}
...
def options_flow(hass, mock_config_entry):
    """Create an OptionsFlowHandler with config_entry patched like HA FlowManager does."""
...
async def test_options_flow_add_zone(
    hass: HomeAssistant, options_flow, mock_entity_registry
):
    ...
    with patch(
        ...
        with patch.object(
            ...
            new_zone = next(
                z
                for z in call_args.kwargs["data"][CONF_ZONES]
...
async def test_options_flow_zone_dict_creation(
    hass: HomeAssistant, options_flow, mock_entity_registry
):
    ...
    with patch(
        ...
        assert any(
            ...
            if hasattr(key, "schema") and CONF_ZONES in str(key)
        )
