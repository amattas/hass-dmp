...
def _make_mock_zone(state="N"):
    """Create a mock pyDMP Zone object."""
    zone = Mock()
    zone.state = state
    zone.is_open = state == "O"
    zone.is_normal = state == "N"
    zone.is_bypassed = state == "X"
    zone.has_fault = state in ("S", "L", "M")
    return zone
...
def test_device_class_mapping(
    setup_sensor, mock_config_entry, zone_class, expected_device_class
):
...
def test_icon_mapping(
    setup_sensor, mock_config_entry, zone_class, state, expected_icon
):
...
def test_battery_sensor_initialization(setup_battery_sensor, mock_config_entry):
    ...
    zone_config = {
        CONF_ZONE_NAME: "Test Battery",
        CONF_ZONE_NUMBER: "010",
        CONF_ZONE_CLASS: "battery_window",
    }
    ...
    panel.ensure_zone.assert_called()


@pytest.mark.parametrize(
    "state,icon", [(False, "mdi:battery"), (True, "mdi:battery-alert-variant-outline")]
)
...
def test_trouble_sensor_initialization(setup_trouble_sensor, mock_config_entry):
    ...
    zone_config = {
        CONF_ZONE_NAME: "Test Trouble",
        CONF_ZONE_NUMBER: "011",
        CONF_ZONE_CLASS: "wired_motion",
    }
...
@pytest.mark.parametrize(
    "state,icon", [(False, "mdi:check"), (True, "mdi:alert-outline")]
)
...
def test_bypass_sensor_initialization(setup_bypass_sensor, mock_config_entry):
    ...
    zone_config = {
        CONF_ZONE_NAME: "Test Bypass",
        CONF_ZONE_NUMBER: "013",
        CONF_ZONE_CLASS: "wired_door",
    }
...
def test_alarm_sensor_initialization(setup_alarm_sensor, mock_config_entry):
    ...
    zone_config = {
        CONF_ZONE_NAME: "Test Alarm",
        CONF_ZONE_NUMBER: "012",
        CONF_ZONE_CLASS: "wired_motion",
    }
...
@pytest.mark.asyncio
@pytest.mark.parametrize(
    "setup_fixture,sensor_cls,zone_name,zone_number,zone_class",
    [
        ("setup_sensor", DMPZoneOpenClose, "Test Door", "001", "wired_door"),
        (
            "setup_battery_sensor",
            DMPZoneBattery,
            "Test Battery",
            "010",
            "battery_window",
        ),
        ("setup_trouble_sensor", DMPZoneTrouble, "Test Trouble", "011", "wired_motion"),
        ("setup_bypass_sensor", DMPZoneBypass, "Test Bypass", "013", "wired_door"),
        ("setup_alarm_sensor", DMPZoneAlarm, "Test Alarm", "012", "wired_motion"),
    ],
)
async def test_sensor_callbacks_general(
    request,
    setup_fixture,
    sensor_cls,
    zone_name,
    ...
    # Set up zone state for callback
    if sensor_cls == DMPZoneOpenClose:
        sensor._zone.is_open = True
    elif sensor_cls == DMPZoneBattery:
        sensor._zone.state = "L"
    elif sensor_cls == DMPZoneTrouble:
        sensor._zone.has_fault = True
    elif sensor_cls == DMPZoneBypass:
        sensor._zone.is_bypassed = True
    elif sensor_cls == DMPZoneAlarm:
        panel.get_alarm = Mock(return_value=True)
...
def test_sensor_properties_general(
    request,
    setup_fixture,
    sensor_cls,
    zone_name,
